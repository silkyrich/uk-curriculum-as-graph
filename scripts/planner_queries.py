...
def query_source_documents(session, key_stage: str, subject: str = '') -> list[dict]:
    """Fetch source documents for this key stage + subject.

    Filters by subject name to avoid cross-join (e.g. English Grammar doc
    appearing for Science planners).
    """
    if subject:
        # Primary: match by subject name AND key stage in the document name
        query = """
...
def fetch_study_context(session, label: str, study_id: str) -> StudyContext | None:
    ...
    # Source documents (filtered by subject to avoid cross-join)
    source_documents = query_source_documents(session, key_stage, subject)
